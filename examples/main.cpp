...
int main()
{
    ...
    std::cout << ollama::chat(Model, message1) << std::endl;
    ...
    std::cout << ollama::generate_embeddings(Model, "Why is the sky blue?") << std::endl;
    ...
    if ( ollama::pull_model(Model) ) std::cout << "Model was pulled" << std::endl;

    // Copy a model by specifying a source model and destination model name.
    if ( ollama::copy_model(Model, "llama3_copy") ) std::cout << "Model was copied" << std::endl;
    ...
    json model_info = ollama::show_model_info(Model);
    ...
    bool model_loaded = ollama::load_model(Model);
    ...
    std::cout << ollama::generate(Model, "Why is the sky blue?") << std::endl;
    ...
    std::cout << ollama::generate(Model, "Why is the sky green?", options) << std::endl;

    std::function<void(const ollama::response&)> response_callback = on_receive_response;  
    ollama::generate(Model, "Why is the sky orange?", response_callback);

    // You can launch the generation in a thread with a callback to use it asynchronously.
    std::thread new_thread( [response_callback, Model]{ ollama::generate(Model, "Why is the sky gray?", response_callback); } );
    ...
    std::cout << my_ollama_server.generate(Model, "Why is the sky blue?") << std::endl;
